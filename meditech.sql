...
-- ============================================================================
-- STORED PROCEDURES: CRUD for all core tables + transactional example
-- ============================================================================
...
CREATE PROCEDURE sp_AddPatientFile(
  IN p_id INT, IN f_name VARCHAR(255),
  IN f_type VARCHAR(100), IN f_size INT, IN f_data LONGBLOB
)
BEGIN
...
CREATE PROCEDURE sp_UpdateProvider(
  IN prov_id INT, IN prov_phone VARCHAR(20), IN prov_email VARCHAR(150)
)
BEGIN
...
CREATE PROCEDURE sp_AddPatientInsurance(
  IN p_id INT, IN prov_id INT, IN pol_num VARCHAR(100),
  IN cov_details TEXT, IN start_dt DATE, IN end_dt DATE
)
BEGIN
...
CREATE PROCEDURE sp_UpdatePatientInsurance(
  IN p_id INT, IN prov_id INT, IN pol_num VARCHAR(100), IN new_end DATE
)
BEGIN
...
CREATE PROCEDURE	sp_AddInvoice(
  IN p_id INT, IN inv_date DATETIME, IN tot_amt DECIMAL(10,2), IN st ENUM('Pending','Paid','Cancelled')
)
BEGIN
...
CREATE PROCEDURE sp_UpdateInvoiceStatus(
  IN inv_id INT, IN new_status ENUM('Pending','Paid','Cancelled')
)
BEGIN
  UPDATE Invoices SET status = new_status WHERE invoice_id = inv_id;
END$$
...
CREATE PROCEDURE sp_AddPayment(
  IN inv_id INT, IN pay_date DATETIME, IN amt_paid DECIMAL(10,2),
  IN method ENUM('Cash','Card','Insurance','Other'), IN tx_ref VARCHAR(100)
)
BEGIN
...
CREATE PROCEDURE sp_CreateInvoiceWithPayment(
  IN p_patient INT, IN p_amount DECIMAL(10,2),
  IN pay_amount DECIMAL(10,2), IN pay_method ENUM('Cash','Card','Insurance','Other')
)
BEGIN
  DECLARE new_inv INT;
  START TRANSACTION;
